/*
    This is simplified and modified from Kissicp(https://github.com/PRBonn/kiss-icp.git)
*/
#include <FastIcp.hpp>
#include <iostream>
...
V3d_i FastIcp::VoxelDownsampleSemantic(const V3d_i &frame,
    ...
    for(int i=0;i<(int)frame.first.size();i++){
        // if(frame.second[i]!=16 && frame.second[i]!=18)
        // {
        ...
        // }
        // else{ // pole , truck
        //     const auto voxel = Eigen::Vector3i((frame.first[i] / voxel_size_pole).cast<int>());
        //     if (grid_pole_like.contains(voxel) ) continue;
        //     grid_pole_like.insert({voxel, i});
        // }
...
Vector4dVectorTuple FastIcp::GetCorrespondences(
    ...
    auto GetClosestNeighboor = [&](const Eigen::Vector4d &point) {
        ...
        std::for_each(neighboors.cbegin(), neighboors.cend(), [&](const Eigen::Vector4d &neighbor) {
            ...
            if (distance < closest_distance2) { //check semantic consisttency
